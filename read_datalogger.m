...
T = readtable("D:\Desktop\RocketLab2023\RocketnRoll_MATLAB_2023\RealTerm\captures\capture.txt"); % input IMU data
...
ax_mean = mean(accel_raw(1:500));
ay_mean = mean(accel_raw(1:500));
az_mean = mean(accel_raw(1:500));
gx_mean=mean(T.gX(1:500));
gy_mean=mean(T.gY(1:500));
gz_mean=mean(T.gZ(1:500));

accel_offset=[ax_mean,ay_mean,az_mean];
gyro_offset=[gx_mean,gy_mean,gz_mean];
...
accel_norm = sqrt(sum(accel_new.*accel_new,2)); % acceleration norm [m/s^2]
...
%  plot(t,accel_raw(:,1)*0.001,'.-','MarkerSize',1,'Color',clr)
%  xlabel('t [s]')
%  ylabel('a_x [m/s^2]')
%  xlim([t0 tf])
%  legend('filtered','raw')
...
% plot(t,accel_raw(:,2)*0.001,'.-','MarkerSize',1,'Color',clr)
% xlabel('t [s]')
% ylabel('a_y [m/s^2]')
% xlim([t0 tf])
% legend('filtered','raw')
...
% plot(t,accel_raw(:,3)*0.001,'.-','MarkerSize',1,'Color',clr)
% xlabel('t [s]')
% ylabel('a_z [m/s^2]')
% xlim([t0 tf])
% legend('filtered','raw')

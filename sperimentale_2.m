...
clear all; close all; clc 
 T = readtable("C:\Users\Utente\Desktop\lancio_primo\prova_lancio_1.txt"); % input IMU data
...
% IMU data filtering
[b_accel,a_accel] = butter(9,accel_fs/(fs*10)); % design order 9 Butterworth filter accelerometer
...
for k=1:(n-1)
    ...
    if quat(1,k+1)<0
        quat(1,k+1) = -quat(1,k+1);
    end
    if quat(2,k+1)<0
        quat(2,k+1) = -quat(2,k+1);
    end
    if quat(3,k+1)<0
        quat(3,k+1) = -quat(3,k+1);
    end
    if quat(4,k+1)<0
        quat(4,k+1) = -quat(4,k+1);
    end
...
[b_pos, a_pos] = butter(1,accel_fs/(fs*10),'high');

...
T = readtable("./lanci_prova/lancio_3/prova_lancio_3.TXT"); % input IMU data
...
t = t(i_t0:i_tf);
accel_raw = accel_raw(i_t0:i_tf,:);

% IMU auxiliary variables
fs=0.1; % sampling frequency [Hz]
% IMU data filtering
[b_accel,a_accel] = butter(9,0.022, 'high'); % design order 9 Butterworth filter accelerometer
%[b_gyro,a_gyro] = butter(9,0.09); % design order 9 Butterworth filter accelerometer
accel_f = filtfilt(b_accel,a_accel,accel_raw);
[b_accel,a_accel] = butter(9,0.85, 'low');
accel_f = filtfilt(b_accel,a_accel,accel_f);
...
for k=1:n-1
    ...
    [b,a] = butter(9,0.03,'low');
    vel_z = filtfilt(b,a,vel_z);

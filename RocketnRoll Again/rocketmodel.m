...
%% Input parameters
% rocket
m0=0.3; % total mass [kg] range: 0.25 - 0.3 kg perchè il propellente brucia ; apogeo max: 200 m
...
[t X]=ode45(@rkt_dyn,[0 100],[0 0 0 0 m0],options); %X vettore delle variabili di stato x,y,vx,vy,m
x=X(:,1);
y=X(:,2);
vx=X(:,3);
vy=X(:,4);
m=X(:,5); %massa finale (considerando repellente consumato a quell'istante)
%% Representation
figure(1) 
plot(tT(:,1),tT(:,2))
xlabel('time [s]')
